...
def generate_launch_description():
    ...
    # launch gazebo
    list_description = [ExecuteProcess(cmd=['gazebo', '-s', 'libgazebo_ros_factory.so'], output='log')]

    # set use_sim_time = true after 5 seconds
    action = ExecuteProcess(cmd=['ros2', 'param', 'set', '/gazebo', 'use_sim_time', 'true'], output='log') #use_sim_time, output = 'screen'
    ...
    list_description.append(DeclareLaunchArgument(
        name='gui',
        default_value='False',
        description='Whether to execute gzclient 2'))
