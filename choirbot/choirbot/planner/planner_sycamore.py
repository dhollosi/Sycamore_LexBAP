...
class Planner(Node):
    ...
    def __init__(self, pos_handler: str = None, pos_topic: str = None):
        ...
        self.task_pos = self.get_parameter('task_position').value
        ...
        self.get_logger().info('Obtained goal in Planner for agent {}: {} '.format(self.agent_id, self.task_pos))
...
class PointToPointPlanner(Planner):
    ...
    def __init__(self, goal_tolerance: float = 0.05, pos_handler: str = None, pos_topic: str = None):
        ...
        # self.goal_point = np.array(self.task_pos)
    ...
    def execute_callback(self, goal_handle):
        ...
        # self.get_logger().info('\n\n ****DEBUG**** Agent {} has a starting position of {} '.format(self.agent_id, self.current_pose.position))
...
class TwoDimPointToPointPlanner(PointToPointPlanner):
    ...
    def send_to_controller(self):
        msg = Point()
        msg.x = float(self.goal_point[0])
        msg.y = float(self.goal_point[1])
        msg.z = 0.0
        self.tocontrol_publisher.publish(msg)
